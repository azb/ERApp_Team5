...
public class CloudBackendFragment extends Fragment {
	...
	/**
	 * Signs in to the application with an account. Subscribes to broadcast
	 * messages once the account picking flow has completed, or immediately if
	 * there is no authentication required.
	 * 
	 * @param overrideCurrent
	 *            {@code true} if user can choose an account even if already
	 *            signed in, {@code false} if the user can choose an account
	 *            only if there is no currently signed in user
	 */
