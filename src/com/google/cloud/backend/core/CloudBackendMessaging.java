...
public class CloudBackendMessaging extends CloudBackendAsync {
	...
	private final Map<String, CloudCallbackHandler<List<CloudEntity>>> mMessageHandlers = new HashMap<String, CloudCallbackHandler<List<CloudEntity>>>();
	 ...
	 *            {@link CloudCallbackHandler} that will handle Cloud Message
	 *            for the topic. It will receive {@link List} of
	 *            {@link CloudEntity}s that contain the messages.
	 * @param maxOfflineMessages
	 *            (optional) if you want to receive all the off-line messages
	 *            sent after the last message received, set max number of the
	 *            messages to receive.
	 */
	public void subscribeToCloudMessage(String topicId, CloudCallbackHandler<List<CloudEntity>> handler,
			int... maxOfflineMessages) {
	...
	private CloudQuery createQueryForCloudMessage(String topicId, int maxOfflineMessages) {
		...
		cq.setFilter(Filter.and(Filter.eq(PROP_TOPIC_ID, topicId), Filter.gt(CloudEntity.PROP_CREATED_AT, new DateTime(lastTime))));
	...
	private class CloudMessageHandler extends CloudCallbackHandler<List<CloudEntity>> {
		...
		public void onComplete(List<CloudEntity> messages) {
			...
			backend.continuousQueries.put(topicId, new ContinuousQueryHandler(cqh.getHandler(), cq, getCredential()));
	 ...
	 *            properties to include in the message
	 * @param handler
	 *            {@link CloudCallbackHandler} that has
